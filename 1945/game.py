"""
A simple shoot-em-up game written in Python and Pygame.
Version: 1.0
Author: sodacrunch
Updater: Robert Chapin
Date: 2012-02-19
Original Github: https://github.com/sodacrunch/1945
"""
import random
import sys
import pygame
...
class Animation:
    """A class for managing simple animations"""
    ...
    def draw(self, dest):
        """Draws the current frame to the destination surface."""
...
class Player(object):
    """A class for the player's plane"""
    def __init__(self):
    ...
    def draw(self, dest):
        """Draws the player's plane to the destination surface."""
        self.anim.pos = self.rect
        self.anim.draw(dest)

    def update(self):
        """Updates the player's plane based on key presses."""
...
class Explosion(object):
    """A class for the explosion animation"""
    ...
    def draw(self, dest):
        """Draws the explosion animation to the destination surface."""
        if self.type == 1:
...
class Enemy(object):
    ...
    def draw(self, dest): 
        """Draws the enemy plane to the destination surface."""
        self.anim.pos = self.rect
        self.anim.draw(dest)
...
class Water(object):
    """A class for the water background"""
    def __init__(self):
...
class Fire(object):
    """A class for the fire animation"""
    def __init__(self, type):
        self.image = fire_1
        self.rect = self.image.get_rect()
        self.rect.x = 100
    ...
    def draw(self, dest):
        """Draws the fire animation to the destination surface."""
        self.anim.pos = self.rect
        self.anim.draw(dest)
...
class Menu(object):
    ...
    def draw(self, dest):
        """Draws the menu to the destination surface."""
...
def create_shot(type, x, y):
    """Creates a shot object and adds it to the shots list."""
    if type == 1:
        f = Fire(type)
        f.rect.x = x + 13
        f.rect.y = y + 40
...
def create_enemy():
    """Creates an enemy object and adds it to the enemies list."""
    e = Enemy()
    random.seed()
...
def create_explosion(type, erectx, erecty):
    """Creates an explosion object and adds it to the explosions list."""
    x = Explosion(type)
    x.rect.x = erectx
    x.rect.y = erecty
    explosions.append(x)
...
def create_background():
    """Creates a background object and adds it to the background list."""
    bg = Background()
...
def create_water():
    """Creates a water object and adds it to the waters list."""
    w = Water()
    waters.append(w)

def update_water():
    """Updates the waters list."""
    for w in waters:
        w.update()
    if len(waters) == 0:
        create_water()
...
def create_player():
    """Creates a player object and adds it to the players list."""
...
def check_plane_hit():
    """Checks for collisions between enemies and players."""
    for e in enemies:
        for p in players:
...
custom_font = pygame.font.Font("./1945/assets/imagine_font.ttf", 18)
white = (255,255,255)
light_grey = (191,191,191)
gold = (255,215,0)

# LOAD GRAPHICS
SPRITE_SHEET_FILE = './1945/assets/1945_sprite_sheet.png'
...
MENU_GRAPHIC = './1945/assets/menu.png'
WATER_BG= './1945/assets/waterbgext.png'
...
# LOAD SOUNDS
fusion_snd = pygame.mixer.Sound('./1945/assets/fusion.ogg')
shot_snd = pygame.mixer.Sound('./1945/assets/shot.ogg')
thump_snd = pygame.mixer.Sound('./1945/assets/thump.ogg')
cannon_snd = pygame.mixer.Sound('./1945/assets/cannon.ogg')
start_snd = pygame.mixer.Sound('./1945/assets/start.ogg')
pygame.mixer.music.load('./1945/assets/music_1.ogg')
PLAY_MUSIC = 1
...
while menu_screen.exit == 0:
    for event in pygame.event.get():
        ...
        if key[pygame.K_RETURN]: # pylint: disable=no-member
            menu_screen.exit = 1
            start_snd.play()
            break
    menu_screen.update()
...
while True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT or event.type == pygame.K_ESCAPE: # pylint: disable=no-member
            sys.exit()
            ...
            if event.key == pygame.K_F1 or event.key == pygame.K_RETURN: # pylint: disable=no-member
                if len(players) == 0:
                    create_player()
